...
public final class XbrlParser {
	...
	private Context.Period parsePeriodFromContext(Element contextElement) throws DOMException, ParseException {
        ...
        if (instantElement != null) {
            Date instantDate = FormatUtils.parseDate(instantElement.getTextContent());
            return new Context.Period(true, instantDate, null);
        } else {
            Date startDate = FormatUtils.parseDate(startDateElement.getTextContent());
            Date endDate = FormatUtils.parseDate(endDateElement.getTextContent());
